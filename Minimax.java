...
public class Minimax {
    ...
    /**
     * Encontra o melhor estado sucessor ao estado atual. Usa a versão clássica do
     * Minimax.
     * 
     * @return devolve um objeto Sucessor. Esse objeto contém o estado sucessor e
    ...
    /**
     * Encontra o melhor estado sucessor ao estado atual. Usa a versão Alfa Beta
     * Pruning do Minimax.
     * 
     * @return devolve um objeto Sucessor. Esse objeto contém o estado sucessor e
     *         sua função de utilidade.
    ...
    /**
     * Conta as células livres do tabuleiro. Corresponde a profundidade máxima da
     * árvore de busca.
     * 
     * @param estado corresponde ao estado atual do tabuleiro
     ...
     * @param caracter corresponde ao caracter (X ou O) com o qual os sucessores
     *                 devem ser gerados
     */
    public void geraVizinhos(char[][][] vizinhos, char[][] estado, char caracter, int[][] posicoes) {
        ...
        // Altera os sucessores incluindo o caracter dado como parâmetro nas posições
        // livre
        for (int v = 0; v < vizinhos.length; v++)
     ...
     * @return devolve -1 se o X ganhou; +1 se o O ganhou; 0 se houve empate e 100
     *         se ainda há jogo
     */
    public int utilidade(char[][] atual, int profundidade) {

        if (vencedorDiagonal(atual, 'X'))
            return -1;
        if (vencedorDiagonal(atual, 'O'))
            return 1;
        if (vencedorLinha(atual, 'X'))
            return -1;
        if (vencedorLinha(atual, 'O'))
            return 1;
        if (vencedorColuna(atual, 'X'))
            return -1;
        if (vencedorColuna(atual, 'O'))
            return 1;
        if (profundidade == 0)
            return 0;
        return 100;
    ...
    public Sucessor algoritmo(char estado[][], boolean jogador, int profundidade) {
        ...
        if (valor != 100)
            return new Sucessor(estado, valor);
